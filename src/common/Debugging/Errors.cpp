...
extern "C" { TC_COMMON_API char const* WUWAssertionFailedMessage = nullptr; }
#define Crash(message) \
    WUWAssertionFailedMessage = strdup(message); \
...
namespace
{
    std::string FormatAssertionMessage(char const* format, va_list args)
    {
        ...
        va_copy(len, args);
        std::int32_t length = vsnprintf(nullptr, 0, format, len);
        va_end(len);
...
namespace WUW
{
    ...
    {
        std::string formattedMessage = Util::FormatMessageUtil("\n{}:{} in {} ASSERTION FAILED:\n  {}\n", file, line, function, message) + debugInfo + '\n';
    ...
    void AbortHandler(int sigval)
    {
        ...
        std::string formattedMessage = Util::FormatMessageUtil("Caught signal {}\n", sigval);
        fprintf(stderr, "%s", formattedMessage.c_str());
        fflush(stderr);
        Crash(formattedMessage.c_str());
    }
